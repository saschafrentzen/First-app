...
/**
 * Abstrakte Basisklasse für Hersteller-APIs
 */
export abstract class BaseFridgeAPI {
  ...
  abstract discover(ipAddress: string): Promise<{ name?: string, model: string, features: string[] } | null>;
  abstract getInventory(deviceId: string): Promise<FridgeInventoryItem[]>;
  abstract getTemperatures(deviceId: string): Promise<FridgeTemperature[]>;
 ...
 */
export class SamsungFamilyHubAPI extends BaseFridgeAPI {
  getBaseUrl(): string {
  ...
  private mapFeatures(capabilities: string[]): SmartFridgeFeature[] {
    ...
    return capabilities
      ...
      .filter(Boolean) as SmartFridgeFeature[];
  }
 ...
 */
export class LGThinQAPI extends BaseFridgeAPI {
  getBaseUrl(): string {
